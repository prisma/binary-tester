...
if test -f "platforms/$name.node.dockerfile"; then
  ...
  docker build -f platforms/$name.node.dockerfile --build-arg IMAGE=$i -t $temp_fetch_image_name .
  ...
  docker build -f platforms/$base_image.node.dockerfile --build-arg IMAGE=$i -t $temp_fetch_image_name .
else
...
if test -f "platforms/$name.run.dockerfile"; then
  ...
  docker build -f platforms/$name.run.dockerfile --build-arg IMAGE=$i -t $temp_run_image_name .
elif test -f "platforms/$base_image.run.dockerfile"; then
  echo "using base dockerfile platforms/$base_image.run.dockerfile for $i"
  docker build -f platforms/$base_image.run.dockerfile --build-arg IMAGE=$i -t $temp_run_image_name .
else
  echo "no custom dockerfile found. note that this often results in errors because dependencies such as node is missing."
