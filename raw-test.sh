...
# raw-test.sh tests a given image by launching a docker image
...
temp_run_image_name="raw_test_$i"

# prepare raw scripts
if test -f "platforms/$name.raw.dockerfile"; then
  echo "using specific dockerfile platforms/$name.raw.dockerfile"
  docker build --no-cache -f "platforms/$name.raw.dockerfile" --build-arg IMAGE="$i" -t "$temp_fetch_image_name" .
elif test -f "platforms/$base_image.raw.dockerfile"; then
  echo "using base dockerfile platforms/$base_image.raw.dockerfile for $i"
  docker build --no-cache -f "platforms/$base_image.raw.dockerfile" --build-arg IMAGE="$i" -t "$temp_fetch_image_name" .
else
  echo "no custom dockerfile found. note that this often results in errors because dependencies such as node is missing."
  echo "if you want to customize steps, please create platforms/$base_image.raw.dockerfile or platforms/$name.raw.dockerfile."
...
echo "running1 $temp_fetch_image_name"
echo "running2 $temp_run_image_name"

docker build --no-cache -f raw.dockerfile . \
